...
Adapted from: data/collector_4h.py
Changes:
  - interval=minute60 (1h)
  - DB: data/upbit_1h.db
  - 페이지 수 4배 (4h 33페이지 → 1h ~130페이지/코인 / 3년)
  - SLEEP 살짝 줄임 (백필 1시간 정도 예상)

사용:
    python -m data.collector_1h_upbit --coin KRW-BTC --days 365
    python -m data.collector_1h_upbit --all --days 1095
"""
...
DB_PATH = Path(__file__).resolve().parent / "upbit_1h.db"
INTERVAL = "minute60"
PAGE_SIZE = 200
RETRY_MAX = 3
RETRY_BACKOFF = 1.5
SLEEP_BETWEEN_PAGES = 0.12   # 4h: 0.15. 1h 가 페이지 4배 → 약간 줄임
SLEEP_BETWEEN_MARKETS = 0.25

logger = logging.getLogger("collector_1h")
...
def collect_market(market: str, days: int = 365 * 3, db_path: Path = DB_PATH) -> int:
    """단일 마켓 1h 백필. 1h 봉 = 일 24봉, 3년 = ~26k봉 = ~130페이지."""
    init_db(db_path)
    ...
    if cur_to <= target_oldest:
        logger.info(f"{market}: 1h already covers down to {oldest}")
        return saved
    ...
    while cur_to > target_oldest:
        ...
        if new_oldest >= cur_to:
            logger.info(f"{market}: 1h reached listing start at {new_oldest}")
            break
        cur_to = new_oldest
        time.sleep(SLEEP_BETWEEN_PAGES)

    logger.info(f"{market}: 1h backfill done, {saved} rows ({pages} pages)")
    return saved
...
def collect_all(markets: list[str], days: int = 365 * 3, db_path: Path = DB_PATH) -> dict:
    ...
    for i, m in enumerate(markets, 1):
        logger.info(f"[{i}/{len(markets)}] 1h {m}")
        try:
            n = collect_market(m, days=days, db_path=db_path)
            results[m] = n
        except Exception as e:
            logger.error(f"{m}: 1h FAIL: {e}")
            results[m] = -1
...
def main():
    parser = argparse.ArgumentParser(description="Upbit KRW 1h 봉 수집기")
    ...
    if args.coin:
        ...
        print(f"Saved {n} rows for {args.coin} (1h)")
    elif args.all:
        markets = get_krw_markets()
        print(f"전체 KRW 1h 백필 {len(markets)}개 시작 (days={args.days})")
        results = collect_all(markets, days=args.days, db_path=db_path)
        ok = sum(1 for v in results.values() if v >= 0)
        fail = sum(1 for v in results.values() if v < 0)
        total = sum(v for v in results.values() if v >= 0)
        print(f"\n=== 1h Done: OK {ok} / FAIL {fail} / Total {total} rows ===")
    elif args.top:
        markets = get_top_markets(args.top)
        results = collect_all(markets, days=args.days, db_path=db_path)
        print(f"1h Done: {len(results)} markets")
    else:
        parser.print_help()
        return
